...
func GetSongById(id string, hostId int64) (*models.Song, error) {
	token, err := config.GetSpotifyTokenObject(hostId)
	if err != nil {
		return nil, err
	}
...
func SearchSongs(search string, hostId int64) (*Tracks, error) {
	token, err := config.GetSpotifyTokenObject(hostId)
	if err != nil {
		return nil, err
	}
