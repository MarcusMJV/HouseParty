...
const handleRoomSubmit = async () => {
  try {
    const response = await fetch(`${apiBaseURL}/room/create`, {
      method: 'POST',
      headers: {
      ...
      <form class="space-y-6 w-100" @submit.prevent="handleRoomSubmit">
        ...
        <div class="flex items-center justify-between p-3 border rounded-lg">
          ...
          <label class="relative inline-flex items-center cursor-pointer">
            ...
            <div
              class="w-11 h-6 bg-slate-700 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-sky-500"
            ></div>
          </label>
        </div>
