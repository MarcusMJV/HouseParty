...
const deleteRoom = async (roomId: string) => {
  try {
    const response = await fetch(`${apiBaseUrl}/room/delete/${roomId}`, {
      method: 'DELETE',
      headers: {
  ...
  <div class="flex flex-col items-center pt-10 min-h-screen font-mono bg-slate-950 relative">
    ...
    <div class="relative w-120 mt-8">
      ...
      <div class="relative flex justify-center">
        <span class="px-2 text-white bg-slate-950 text-sm"> {{ username }}'s Room </span>
      </div>
    </div>
    ...
    <div class="flex flex-col items-center mt-4 space-y-4">
      <div
        v-if="userRoom.id != ''"
        class="w-150 bg-slate-900 p-4 rounded-2xl shadow-lg flex items-center justify-between relative"
      >
        <button
          @click="deleteRoom(userRoom.id)"
          class="absolute -top-1 -right-1 text-white hover:text-red-500 hover:cursor-pointer text-xs font-bold p-1 rounded-full transition-colors"
        >
          x
        </button>
        ...
        <RouterLink
          :to="{ name: 'join-room', params: { id: userRoom.id } }"
    ...
    <router-link
      v-if="isSpotifyConnected && userRoom.id == ''"
      to="/create/room"
    ...
    <button
      ...
      @click="requestSpotifyAuth"
      class="p-2 hover:cursor-pointer hover:bg-sky-500/100 border text-white font-semibold py-3 rounded-lg hover:bg-sky-600 transition duration-300"
    >
    ...
    <div class="relative w-120 mt-4">
      ...
      <div class="relative flex justify-center">
        <span class="px-2 text-white bg-slate-950 text-sm"> Public Rooms </span>
      </div>
    </div>
    ...
    <div class="flex flex-col items-center mt-4 space-y-4">
      ...
      <div
        ...
        :key="room.id"
        class="w-150 bg-slate-900 p-4 rounded-2xl shadow-lg mt-4 flex items-center justify-between relative"
      >
        <span
          class="absolute -top-5 text-white text-xs font-bold p-1 rounded-full transition-colors"
          >{{ room.host_name }}</span
        >
        ...
        <RouterLink
          :to="{ name: 'join-room', params: { id: room.id } }"
          class="p-2 hover:cursor-pointer hover:bg-sky-500/100 border text-white font-semibold rounded-lg hover:bg-sky-600 transition duration-300"
        >
          Join
        </RouterLink>
